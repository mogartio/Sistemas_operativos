...
expand_environ_var(char *arg)
{
	if (arg[0] == '$') {
		...
		char *env_var_name = arg + 1;
		char *env_var_value = getenv(env_var_name);
		if (env_var_value != NULL) {
			size_t new_size = strlen(env_var_value) + strlen(arg) + 1;
			char *expanded_arg = (char *) realloc(arg, new_size);
			memmove(expanded_arg + strlen(env_var_value),
			        expanded_arg + strlen(env_var_name) + 1,
			        strlen(expanded_arg) - strlen(env_var_name));
			memcpy(expanded_arg, env_var_value, strlen(env_var_value));
			return expanded_arg;
		} else {
			return strdup("");
		}
